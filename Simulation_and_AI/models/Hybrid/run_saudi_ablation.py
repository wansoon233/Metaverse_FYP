...
SAUDI_WEATHER = '/home/wansoon/Desktop/FYP_Metaverse/sinergym_source/sinergym/data/weather/Saudi.epw'
TIMESTEPS_TEST = 35040 
MODEL_PATH = "/home/wansoon/Desktop/FYP_Metaverse/models/Hybrid/saved_models/ppo_lstm.zip"

print("="*60)
print("🏜️  SAUDI ARABIA ABLATION STUDY (EXTREME HEAT TEST)")
print("="*60)
...
def run_ablation(config_name, use_lstm, use_shield):
    print(f"\n⏳ Running Saudi Test: {config_name}")
    env = gym.make(ENV_NAME, weather_files=SAUDI_WEATHER)
...
results = []
results.append(["PPO-only", *run_ablation("1. PPO Only", False, False)])
results.append(["PPO + Shield", *run_ablation("2. PPO + Shield", False, True)])
results.append(["LSTM + PPO", *run_ablation("3. LSTM + PPO", True, False)])
results.append(["Full Hybrid", *run_ablation("4. Full Hybrid", True, True)])
